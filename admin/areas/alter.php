<?php
	...
	function resize_image($imgname){
		...
		updateAreas(TRUE, $imgname);
	}

	function updateAreas($imgChange, $imgname){
		global $mysqli;
		$nome = $_POST["nome"];
		session_start();
		...
		if ($imgChange){
			$query = "UPDATE `areas`
						SET `nome` = ?, `idusuario_alteracao` = ?, `data_alteracao` = ?, `descricao` = ?, `ativo` = ?, `imagem` = ?
						WHERE `id` = ?";
			if ($stmt = $mysqli->prepare($query)){
				$stmt->bind_param('sissssi', $nome, $idusuario_alteracao, $data_alteracao, $descricao, $ativo, $imgname, $id);
				$stmt->execute();
				if ($stmt->affected_rows == 1){
					header("location: ../index_admin.php?link=60&edit=1");
					die("");
				} else {
					header("location: ../index_admin.php?link=60&edit=0");
					die("");
			...
			$query = "UPDATE `areas`
						SET `nome` = ?, `idusuario_alteracao` = ?, `data_alteracao` = ?, `descricao` = ?, `ativo` = ?
						WHERE `id` = ?";
			if ($stmt = $mysqli->prepare($query)){
				$stmt->bind_param('sisssi', $nome, $idusuario_alteracao, $data_alteracao, $descricao, $ativo, $id);
				$stmt->execute();
				if ($stmt->affected_rows == 1){
					header("location: ../index_admin.php?link=60&edit=1");
					die("");
