<?php
	...
	function resize_image($imgname){
		...
		if($width_orig < $width or $height_orig < $height){
			header("location: ../index_admin.php?link=40&add=2");
			die("");
		}
		...
		updatePublicacoes(TRUE, $imgname);
	}

	function updatePublicacoes($imgChange, $imgname){
		global $mysqli;
		$titulo = $_POST["titulo"];
		$link = $_POST["link"];
		...
		if ($imgChange){
			$query = "UPDATE `publicacoes`
					SET `titulo` = ?, `link` = ?, `descricao` = ?, `imagem` = ?
					WHERE `id` = ?";
			if ($stmt = $mysqli->prepare($query)){
				$stmt->bind_param('ssssi', $titulo, $link, $descricao, $imgname, $id);
				$stmt->execute();
				if ($stmt->affected_rows == 1){
					header("location: ../index_admin.php?link=40&edit=1");
					die("");
				} else {
					echo 'ta aqui .1<br/>';
					echo $imgChange.'<br/>';
			...
			$query = "UPDATE `publicacoes`
					SET `titulo` = ?, `link` = ?, `descricao` = ?
					WHERE `id` = ?";
			if ($stmt = $mysqli->prepare($query)){
				$stmt->bind_param('sssi', $titulo, $link, $descricao, $id);
				$stmt->execute();
				if ($stmt->affected_rows == 1){
					header("location: ../index_admin.php?link=40&edit=1");
					die("");
