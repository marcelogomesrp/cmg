<?php
	if($_SERVER["REQUEST_METHOD"]=="POST"){
		...
		if (!isset($_SESSION['userid'])){
			...
			header("location: ../index_admin.php?link=40&add=0");
			die("");
		}
	...
	function resize_image($imgname){
		...
		insertPublicacoes($imgname);
	}

	function insertPublicacoes($imgname){
		global $mysqli;
		$titulo = $_POST["titulo"];
		$link = $_POST["link"];
		...
		$query = "INSERT INTO `publicacoes`
					(`titulo`, `link`, `descricao`, `imagem`)
					VALUES (?, ?, ?, ?)";
		if ($stmt = $mysqli->prepare($query)){
			$stmt->bind_param('ssss', $titulo, $link, $descricao, $imgname);
			$stmt->execute();
		}
		if ($stmt->affected_rows == 1){
			header("location: ../index_admin.php?link=40&add=1");
			die("");
		} else {
			header("location: ../index_admin.php?link=40&add=0");
			die("");
